...
from datasets.VirtualData.dataset import PoseDataset as PoseDataset_virtual


parser = argparse.ArgumentParser()

parser.add_argument('--dataset', type=str, default='VirtualData', help='ycb or linemod')# virtual
...
'''
1.随机 路径 设置网络 loss  优化器 数据集等准备工作
2.train
3.test
4.中间会改一些参数 写在最后
'''
def main():
    ...
    # 路径
    opt.dataset_root = 'datasets/'+ opt.dataset + '/data'
    opt.num_objects = 1
    opt.num_points = 1000
    opt.result_dir = 'results/virtual'
    opt.repeat_epoch = 10
    # print('unknown dataset')
    # return

    if opt.dataset == 'VirtualData':
        dataset = PoseDataset_virtual('train', opt.num_points, True, opt.dataset_root, opt.noise_trans)
        test_dataset = PoseDataset_virtual('test', opt.num_points, False, opt.dataset_root, 0.0)
    ...
    tb_writer = tf.compat.v1.summary.FileWriter(opt.result_dir)
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                ...
                loss, loss_r, loss_t, loss_reg = criterion(pred_r=pred_r, pred_t=pred_t, pred_c=pred_c, target_r=target_r,
                                                           target_t=target_t, model_points=model_points,idx=idx, obj_diameter=obj_diameter)
                ...
                if train_count % opt.batch_size == 0:
                    ...
                    # tf.Summary
                    summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag='learning_rate', simple_value=lr),
                                                          tf.compat.v1.Summary.Value(tag='loss',
                                                                           simple_value=train_loss_avg / opt.batch_size),
                                                          tf.compat.v1.Summary.Value(tag='loss_r',
                                                                           simple_value=train_loss_r_avg / opt.batch_size),
                                                          tf.compat.v1.Summary.Value(tag='loss_t',
                                                                           simple_value=train_loss_t_avg / opt.batch_size),
                                                          tf.compat.v1.Summary.Value(tag='loss_reg',
                                                                           simple_value=train_loss_reg_avg / opt.batch_size)])
                    tb_writer.add_summary(summary, global_step)
                    logger.info('Train time {0} Epoch {1} Batch {2} Frame {3} Avg_loss:{4:f}'.format(
        ...
        for j, data in enumerate(testdataloader, 0):
            ...
            logger.info('Test time {0} Test Frame No.{1} loss:{2:f} confidence:{3:f} distance:{4:f}'.format(
                time.strftime("%Hh %Mm %Ss", time.gmtime(time.time() - st_time)), test_count, loss, how_min[0].item(),
                dis))
        ...
        for i in range(opt.num_objects):
            ...
            logger.info(
                'success rate: {0}'.format(float(success_count[i]) / num_count[i]))
        accuracy = accuracy / opt.num_objects
        test_dis = test_dis / test_count
        # log results
        logger.info(
            'Test time {0} Epoch {1} TEST FINISH Avg dis: {2:f}, Accuracy: {3:f}'.format(time.strftime("%Hh %Mm %Ss",
                                                                                                       time.gmtime(
                                                                                                           time.time() - st_time)),
                                                                                         epoch, test_dis, accuracy))
        # tensorboard
        summary = tf.compat.v1.Summary(value=[tf.Summary.Value(tag='accuracy', simple_value=accuracy),
        ...
        torch.save(estimator.state_dict(),
                   '{0}/pose_model_{1:02d}_{2:06f}.pth'.format(opt.result_dir, epoch, best_test))
